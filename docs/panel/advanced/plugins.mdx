...
If your plugin adds new default languages to the panel you need to set the plugin category to `languge`. This way translations will be registered without any namespace.
...
Most of the default panel resources and pages have static methods to register custom data or to change existing data. You can call these static methods inside the `register` function of a service provider.
...
public function register(): void
{
    ...
    Role::registerCustomDefaultPermissions([
        'myModel' => [
            'customPerm1',
            'customPerm2'
        ]
...
If you need additional composer packages as dependency for your panel you can add the package name to the `composer_packages` array in your `plugin.json`:
...
<Admonition type="info" title="Package Version">
    Currently you cannot specify the package version. It will use the latest version.
</Admonition>
